...
parser.add_argument('--delete', action='store_true', help='Delete words instead of replacing with silence (default is False). Not available for video.')
...
parser.add_argument('--pre_split', action='store_true', help="Can be used if file is already split so that we don't spend time processing it again.")
...
if resume:
    ...
    if media_type == "audio":
        ...
        if not os.path.exists(sound_file_path):
            ffmpeg.input(file_path).output(sound_file_path, loglevel="error").run(overwrite_output=True)
    ...
    if not pre_split:
        ...
        analysis_files = find_split_files(tmp_dir)#search our temp directory and locate all segment files and place them in an array
...
if len(discovered_bad_words_list) > 0:
    ...
    if media_type == "audio":
        output_audio_file = delete_audio_using_word_match_list(sound_file_path, discovered_bad_words_list, bleep, delete)
    ...
    if media_type == "video": #replace original video audio with edited audio
        if bleep:        
            output_audio_file = delete_audio_using_word_match_list(sound_file_path, discovered_bad_words_list, bleep)#do not delete audio. We need either bleep or silence
            ...
            output_vid_path = delete_audio_using_word_match_list(file_path, discovered_bad_words_list)
