...
from splitter import split_for_analysis, find_split_files
...
parser.add_argument("--debug", action="store_true", help="Export full transcript to text file")

parser.add_argument(
    "--job_id",
    type=str,
    help="The unique ID for the current job (used for status tracking)",
)
...
parser.add_argument(
    "--bleep",
    action="store_true",
    help="Bleep out words instead of replacing with silence (default is False)",
)
parser.add_argument(
    "--delete",
    action="store_true",
    help="Delete words instead of replacing with silence (default is False). Not available for video.",
)
parser.add_argument(
    "--analysis_chunk_size",
    type=int,
    default=60,
    help="How many seconds of audio to split your chunks into for analysis (defaults to 60 seconds [1 minute]) less than 15 seconds may be inaccurate",
)
...
parser.add_argument(
    "--pre_split",
    action="store_true",
    help="Can be used if file is already split so that we don't spend time processing it again.",
)
parser.add_argument(
    "--resume",
    action="store_true",
    help="Resume a previous run (input audio filename must remain same)",
)
parser.add_argument(
    "--transcript",
    type=str,
    help="Path to transcript (optional, improves accuracy if transcript is correct)",
)
...
bleep = args.bleep or (env_censor_method == "bleep")
delete = args.delete or (env_censor_method == "delete")
analysis_chunk_size = int(os.getenv("ANALYSIS_CHUNK_SIZE", args.analysis_chunk_size))
...
if len(bad_words) < 1:
    print(
        "It seems that your bad words list is empty. Please add words to it that you would like to detect and remove."
    )
    sys.exit(1)  # stop further execution
    logging.info("number of bad words in list %s", len(bad_words))
...
if resume:
    ...
    if os.path.exists(data_file):
        with open(data_file, "rb") as pickle_file:
            ...
            if len(discovered_words_list) < 1:
                print(
                    "Error loading data. Please run again without --resume to re-generate"
                )
                sys.exit(1)  # stop further execution
            print(f"data successfully loaded from {data_file}")
    else:
        print(
            "Error! Resume selected, but data not present. Please remove --resume flag and re-run to generate data."
        )
        sys.exit(1)  # stop further execution
    ...
    if media_type == "audio":
        ...
        if not os.path.exists(sound_file_path):
            ffmpeg.input(file_path).output(sound_file_path, loglevel="error").run(
    ...
    if not pre_split:
        analysis_files = split_for_analysis(
            sound_file_path, tmp_dir, analysis_chunk_size
        )
    else:
        analysis_files = find_split_files(
            tmp_dir
        )  # search our temp directory and locate all segment files and place them in an array
...
if len(discovered_words_list) == 0:  # no existing data loaded
    ...
    discovered_words_list = find_all_words_in_all_segments(
        analysis_files, tmp_dir, transcript_path, job_id=job_id
    )

    with (
        open(discovered_words_file_export_name, "wb") as pickle_file
    ):  # save our data so that we can resume later if we want to or something crashes.
...
if args.debug:
    # Generate a simple text transcript from the word list
    transcript_file = os.path.splitext(os.path.basename(file_path))[0] + "_transcript.txt"
    with open(transcript_file, "w") as f:
        # Join words with spaces, adding a newline every 20 words for readability
        for i, item in enumerate(discovered_words_list):
            f.write(item["word"] + " ")
            if (i + 1) % 20 == 0:
                f.write("\n")
    print(f"Debug mode: Transcript exported to {transcript_file}")

if len(discovered_bad_words_list) > 0:
    print(len(discovered_bad_words_list), "bad words discovered.")

    if no_edit is True:
        print("No edit was specified. Analysis is complete. Stopping.")
        sys.exit(1)  # stop further execution
    ...
    if media_type == "audio":
        output_audio_file = delete_audio_using_word_match_list(
            sound_file_path, discovered_bad_words_list, bleep, delete
        )
    ...
    if media_type == "video":  # replace original video audio with edited audio
        if bleep:
            output_audio_file = delete_audio_using_word_match_list(
                sound_file_path, discovered_bad_words_list, bleep
            )  # do not delete audio. We need either bleep or silence
            output_vid_path = (
            ...
            output_vid_path = delete_audio_using_word_match_list(
                file_path, discovered_bad_words_list
            )
        print(f"Video edited succesfully. Saved to {output_vid_path}")

else:
    print(
        "no inapproriate words found to censor. If this has false negatives, adding a transcript may improve accuracy. Note: small chunk sizes may lead to inaccurate results as well"
    )
