...
def split_for_analysis(input_file, output_dir, segment_length_s):
    print(
        f"splitting audio file into smaller pieces of {segment_length_s} seconds each for easier analysis"
    )
    ...
    print(
        f"audio file will be split into {num_segments} segments with a max length of {segment_length_s} seconds each"
    )
    ...
    timer_start_time = (
        time.time()
    )  # log start time so that we can estimate remaining time
    ...
    for i in range(num_segments):
        ...
        if estimated_time_remaining > 3600:
            ...
            print(
                f"Iteration {i + 1}/{num_segments} - Elapsed Time: {elapsed_time:.2f} seconds - Estimated Time Remaining: {hours_remaining:.2f} hours",
                end="\r",
            )
        elif estimated_time_remaining > 60:
            minutes_remaining = estimated_time_remaining / 60
            print(
                f"Iteration {i + 1}/{num_segments} - Elapsed Time: {elapsed_time:.2f} seconds - Estimated Time Remaining: {minutes_remaining:.2f} minutes",
                end="\r",
            )
        else:
            print(
                f"Iteration {i + 1}/{num_segments} - Elapsed Time: {elapsed_time:.2f} seconds - Estimated Time Remaining: {estimated_time_remaining:.2f} seconds",
                end="\r",
            )
    ...
    total_elapsed_time = timer_end_time - timer_start_time
...
def find_split_files(
    ...
    matching_files_full_path = [
        search_directory + "/" + item for item in matching_files
    ]
