...
def delete_audio_using_word_match_list(
    audio_file_path, match_list, bleep=False, delete=False
):
    ...
    if bleep:
        ...
        for index, match in enumerate(match_list):
            ...
            sine_filters.append(
                f"frequency: 1000, duration: {duration}, start_time: {start_time}"
            )
        # sine_filters = [
        ...
        (ffmpeg.input(audio_file_path).output(filter_complex, out_file).run())
        ...
        for index, match in enumerate(match_list):
            ...
            if index == 0:
                ...
                filter_complex += (
                    f"[0:a]atrim=start=0:end_sample={start_sample}[clip{index}_1],"
                )
                n += 1
            ...
            ):  # on the last iteration start from the end of our word and go until the end
                filter_complex += (
                    f"[0:a]atrim=start_sample={end_sample}[clip{index}_2],"
                )
                n += 1
